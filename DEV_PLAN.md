...
**account 账户**
- id, ledger_id, type（asset 资金 / credit 信贷 / stored_value 储值）, name, icon, initial_balance, balance（当前余额）, status, created_at
...
- `GET /api/bills/{id}/logs` 操作记录（返回操作人昵称 + 可读摘要，`BillLogVO`）
...
/                         主框架（侧边栏 + 顶栏，路由嵌套）
  ...
  ├── /accounts            账户资产（总览 + 按 资金/信贷/储值 分组列表）
  ├── /budgets             预算总览
  ├── /statistics          统计（趋势 + 占比）
  ├── /family              家庭（成员列表 + 邀请，入口在右上角用户下拉）
  └── /settings            设置（账户信息 / 成员查看 / 账本管理）

> 说明：创建 / 加入家庭为全局弹框（FamilySetupDialog），无独立引导页；
> 左侧导航不设"家庭"，家庭入口在右上角用户下拉菜单。
```
...
- [x] 前后端脚手架：Vue3+Vite+TS / Spring Boot3+Java17，目录结构、统一响应体、全局异常处理
- [x] Git 仓库初始化、`.gitignore`、README、分支规范
- [x] MySQL 建库 + 建表脚本（`sql/init.sql`）+ 基础数据（支出 / 收入预设分类）
- [x] Redis 接入；MyBatis-Plus 集成；Swagger 配置
- [x] 统一前端请求封装（axios 拦截器、token 注入、401 处理）、路由骨架
- **验收**：前后端能各自启动，`GET /api/health` 通，页面骨架可访问。

### Phase 1 — 认证与用户（约 5~6 天）
- [x] 手机号验证码：发送接口（接短信服务 / 先接 Redis 存储 + 开发环境 console 输出）、登录接口、JWT 签发
- [x] 微信登录：仅预留接口与功能开关（资质具备后接入，v1 不阻塞）
- [x] 用户表读写、`/api/auth/me`
- [x] 前端登录页（手机号验证码为主，微信入口置灰提示"即将开放"）、登录态持久化（Pinia + localStorage）
- **验收**：手机号验证码可登录，刷新页面登录态不丢，退出登录生效。

### Phase 2 — 家庭与成员（约 3~4 天）
- [x] 家庭 CRUD、邀请码生成、凭码加入
- [x] 成员列表 / 角色
- [x] 前端首次引导页（创建 / 加入家庭）、家庭页（成员列表 + 邀请）
- **验收**：A 创建家庭 → 生成邀请码 → B 登录后凭码加入 → 双方看到同一家庭成员。

### Phase 3 — 账本与基础数据（约 4~5 天）
- [x] 账本 CRUD（公共 / 个人）、账本成员关系、当前账本概念
- [x] 分类管理、标签管理（创建者权限校验）
- [x] 前端账本列表 / 新建 / 详情骨架、账本管理页、顶部账本切换器
- **验收**：建公共账本并邀请成员后，成员可见该账本；个人账本仅本人可见；创建者可增删分类 / 标签，普通成员无入口。

### Phase 4 — 记账核心（约 8~10 天，重点）
- [x] 账单 CRUD + 多账户明细事务（bill_account）
- [x] 支出多账户组合付款（前端金额分配 + 后端校验分配合计 = 总额）
- [x] 转账（pair 配对，双边余额增减）
- [x] 记账人字段：公共账本可代记，个人账本固定本人
- [x] 分类图标网格、日期（精确到时分）、备注、标签多选
- [x] 连续记账（保留上次账户 / 标签）
- [x] 账单列表：日期分组、筛选（类型 / 分类 / 账户 / 成员 / 日期）、搜索、排序
- [x] 操作留痕：改 / 删写 audit_log，账单详情展示操作记录（时间线卡片：操作类型标签 + 可读摘要 + 操作人 + 时间）
- **验收**：完整走通"建账本 → 记支出（多账户分配）→ 记收入 → 记转账 → 编辑 → 删除"，账户余额随之正确变化；改删有记录；组合付款分配不等于总额被拦截。

### Phase 5 — 账户资产（约 4~5 天）
- [x] 账户 CRUD（三类账户：资金 asset / 信贷 credit / 储值 stored_value）、信贷账户视觉区分
- [x] 新增账户：类型下推荐账户快捷标签（点击填充名称）+ 自定义名称入口
- [x] 余额维护 + account_balance_log 留痕
- [x] 资产总览（总资产 / 总负债 / 净资产，资金+储值=资产、信贷=负债）、账户详情（流水 + 调整记录）
- [x] 前端账户页 / 新增 / 详情
- **验收**：资产总览数值 = 各账户余额汇总；信贷账户红/橙展示；调整余额有原因与记录。

### Phase 6 — 预算（约 3~4 天）
- [x] 预算 CRUD（按分类、自定义周期）
- [x] 预算进度计算（按周期聚合分类支出）、超支标记
- [x] 前端预算总览（进度条、超支标红）、新建 / 编辑
- **验收**：月度餐饮预算 2000，本月餐饮支出超 2000 后预算页标红并提示。

### Phase 7 — 统计（约 3~4 天）
- [x] 后端聚合接口：趋势（日 / 周 / 月）、分类占比
- [x] ECharts 集成：折线 / 柱状趋势图、饼图，筛选器（账本 / 时间 / 收 / 支）
- **验收**：图表数据与账单流水一致，切换时间范围与账本数据正确刷新。

### Phase 8 — 设置与收尾（约 4~6 天）
- [x] 设置页：账户信息只读、成员查看、账本管理（新建 / 删除二次确认）
- [x] 空状态全量补齐、加载态、全局错误提示
- [x] 权限可见性终检（非创建者无管理入口）
- [x] 前后端联调、边界用例（删除账本级联、并发记账、金额精度 BigDecimal）
- [x] 部署上线：Nginx + 后端部署 + HTTPS 域名（微信登录正式配置待资质到位后补）
- [x] 编写部署文档 `docs/DEPLOY.md` 与 README 使用说明
...
| 日期 | 提交 | 内容 |
|---|---|---|
| 2026-08-27 | `39ad9b1` | 家庭入口迁移：左侧导航移除"家庭"，入口移至右上角用户下拉；创建 / 加入家庭改为全局弹框（`FamilySetupDialog`），移除 `OnboardingView` 引导页 |
| 2026-08-28 | `40d997f` | 账户类型重构为三分类（资金 asset / 信贷 credit / 储值 stored_value）+ 新增账户推荐快捷选择与自定义入口；账单操作记录接口返回操作人 + 可读摘要（`BillLogVO`），详情页改为时间线卡片展示 |
